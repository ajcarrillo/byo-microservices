 ...
 * @param {string} address The product address
 * @param {boolean} includeFileDetails Include image and file details
 * @return {Promise} A product object
 */
const getShopProductByAddress = (address, includeFileDetails) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (results.length > 0) {
        const imgList = results[0].product_image_addresses.split(',').join("','")
        const images = await getProductImageList(`'${imgList}'`)
        const pImages = images.map((i) => {
          if (!includeFileDetails) {
            return `${process.env.CDN_SHOP_RESOURCES}${i.location.replace(/\\/g, '/')}/${i.stored_name}`
          } else {
            return {
              address: i.address,
              name: i.orig_name,
              url: `${process.env.CDN_SHOP_RESOURCES}${i.location.replace(/\\/g, '/')}/${i.stored_name}`,
            }
          }
        })

        let productFile
        if (includeFileDetails && results[0].product_file_address) {
          const pFile = await getProductFileByAddress(results[0].product_file_address)
          productFile = {
            address: pFile[0].address,
            name: pFile[0].orig_name,
            url: `${process.env.CDN_SHOP_RESOURCES}${pFile[0].location.replace(/\\/g, '/')}/${pFile[0].stored_name}`,
          }
        }
        ...
        const product = {
          ...
          productStockLevel: results[0].product_stock_level,
          productGroups: results[0].product_group_addresses.split(','),
          ...(includeFileDetails && productFile && {productFile}),
        }
        resolve(product)
      } else {
        resolve(null)
 ...
 * @param {string} groupAddress The group address
 * @param {string} groupName The group name
 * @param {string} groupDescription The group description
 * @param {boolean} replaceImage Replace the image true/false
 * @param {string} originalName The original name of the image
 * @param {string} key The new file name and folder for the image
 * @param {string} size The image file size
 * @param {string} mimetype The image mime type
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (replaceImage) {
        ...
        const filename = keyParts[keyParts.length-1]
        const uid = await Auth.createUniqueID()
        imgAddress = Auth.generateAddress(uid + filename)
        origName = mysqlRealEscapeString(originalName)
        fName = mysqlRealEscapeString(filename)
        dest = mysqlRealEscapeString(`/${process.env.SHOP_BUCKET_GROUP_IMAGES_FOLDER}`)
      }
      ...
      const shopGroups = await getShopGroups()
      if (shopGroups.some((g) => g.group_name === gName && g.group_address !== groupAddress)) {
        if (replaceImage) {
          await deleteBucketItem(
            process.env.BYOWAVE_RESOURCES_SHOP_BUCKET_ENDPOINT,
            process.env.BYOWAVE_RESOURCES_SHOP_BUCKET_NAME,
        ...
        const existingGroup = shopGroups.find((eG) => eG.group_address === groupAddress)
        if (existingGroup) {
          if (replaceImage) {
            // Remove old image
            const img = await getGroupImage(existingGroup.group_image_address)
            await deleteBucketItem(
              process.env.BYOWAVE_RESOURCES_SHOP_BUCKET_ENDPOINT,
              process.env.BYOWAVE_RESOURCES_SHOP_BUCKET_NAME,
              `${img[0].location.slice(1)}/${img[0].stored_name}`,
            )
            await deleteShopImage(existingGroup.group_image_address)

            // Create new image
            await createNewShopImage(imgAddress, origName, fName, mimetype, dest, size)
          }

          const imgToSave = replaceImage ? imgAddress : existingGroup.group_image_address
          await updateShopGroup(groupAddress, imgToSave, gName, gDesc)
          resolve()
        } else {
          reject(new Error('GROUP_NOT_FOUND'))
        }
      }
    } catch (e) {
      logger.error(`shop-lib: Failed to update shop group with address ${groupAddress}: ${e}`)
      reject(new Error('ERROR'))
    }
  })
}
...
/**
 * Updates an existing shop product
 * @param {string} metaTitle Meta ttile
 * @param {string} metaDescription Meta description
 * @param {string} metaKeywords Meta keywords
 ...
 * @param {string} productDispatchTime Product dispatch time
 * @param {string} productImages Product image addresses
 * @param {string} existingProductImages Existing product image addresses
 * @param {string} productGroups Product group addresses
 * @param {string} productFile Product file address
 * @param {string} existingProductFile Existing product file address
 * @return {Promise}
 */
const updateExistingShopProduct = (
  metaTitle,
  metaDescription,
  metaKeywords,
  productAddress,
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const savedProduct = await getProductByCode(pCode)

      const pAddress = mysqlRealEscapeString(productAddress)
      const mTitle = mysqlRealEscapeString(metaTitle)
      const mDesc = mysqlRealEscapeString(metaDescription)
      const mKey = mysqlRealEscapeString(metaKeywords)
      const pName = mysqlRealEscapeString(productName)
      ...
      const pDisp = mysqlRealEscapeString(productDispatchTime)
      const pImg = mysqlRealEscapeString(productImages)
      const epImg = mysqlRealEscapeString(existingProductImages)
      const pGrp = mysqlRealEscapeString(productGroups)
      const pFile = mysqlRealEscapeString(productFile)
      const epFile = mysqlRealEscapeString(existingProductFile)

      // Remove any deleted images
      const existingImages = savedProduct[0].product_image_addresses.split(',')
      const imagesToKeep = epImg.split(',')
      for (let eI = 0; eI < existingImages.length; eI++) {
        if (!imagesToKeep.includes(existingImages[eI])) {
          const img = await getProductImage(existingImages[eI])
          await deleteBucketItem(
            process.env.BYOWAVE_RESOURCES_SHOP_BUCKET_ENDPOINT,
            process.env.BYOWAVE_RESOURCES_SHOP_BUCKET_NAME,
            `${img[0].location.slice(1)}/${img[0].stored_name}`,
          )
          await deleteShopImage(existingImages[eI])
        }
      }

      // New image list
      const newImgs = pImg ? pImg.split(',') : []
      const oldImgs = epImg ? epImg.split(',') : []
      const imgsToSave = newImgs.concat(oldImgs).join(',')

      // Remove an old product file if it is being replaced
      if (pFile || !epFile) {
        const fileToRemove = await getProductFile(pCode)
        if (fileToRemove.length > 0) {
          await deleteBucketItem(
            process.env.BYOWAVE_RESOURCES_SHOP_BUCKET_ENDPOINT,
            process.env.BYOWAVE_RESOURCES_SHOP_BUCKET_NAME,
            `${fileToRemove[0].location.slice(1)}/${fileToRemove[0].stored_name}`,
          )
          await deleteProductFile(fileToRemove[0].address)
        }
      }

      const fileToSave = epFile ? epFile : pFile ? pFile : ''
      ...
      logger.error(`shop-lib: Failed to update shop product with code ${productCode}: ${e}`)
      reject(new Error('ERROR'))
    }
  })
}
...
export {
  newShopGroup,
  updateExistingShopGroup,
  newShopProduct,
  updateExistingShopProduct,
  newShopProductImage,
  newShopProductFile,
