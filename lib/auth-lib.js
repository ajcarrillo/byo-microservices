      ...
      if (results.length > 0) {
        ...
        if (authorised) {
          ...
          resolve({
            ...
            localisation: results[0].language_code || '',
          })
        } else {
...
/**
 * Checks if an email address has administrator privileges
 * @param {string} email Email address
 * @return {Promise}
 */
const isUserAdministrator = (email) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (admins.some((a) => a.email_address === email)) {
        resolve(true)
      } else {
        return reject(new Error('NOT_ADMIN'))
      }
    } catch (e) {
      logger.error(`auth-lib: Failed to request admin access for email address ${email}: ${e}`)
      reject(e)
    }
  })
}
