 ...
 * @param {string} key The new file name and folder for the controller image
...
const newProteusController = (
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const dest = mysqlRealEscapeString(`/${process.env.PROTEUS_BUCKET_CONTROLLER_IMAGES_FOLDER}`)
      ...
      resolve({
        personal: personal.map((c) => {
          return {
            ...
            image: `${process.env.CDN_PROTEUS_RESOURCES}${c.location.replace(/\\/g, '/')}/${c.stored_name}`,
            name: c.config_name,
    ...
    try {
      ...
      if (c.length > 0) {
        resolve({
          ...
          image: `${process.env.CDN_PROTEUS_RESOURCES}${c[0].location.replace(/\\/g, '/')}/${c[0].stored_name}`,
